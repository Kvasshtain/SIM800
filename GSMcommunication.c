...
const char SAVE_ACT_STATE_0_CMD[] = "akt sost 0 vhoda:";
const char SAVE_ACT_STATE_1_CMD[] = "akt sost 1 vhoda:";
const char SAVE_ACT_STATE_RNG_0_CMD[] = "akt sost 0 vhodov:";
const char SAVE_ACT_STATE_RNG_1_CMD[] = "akt sost 1 vhodov:";
...
void Save_Alarm_State(uint8_t state)
{
	...
	if (strchr(input_num_pos, ':') == NULL) // если пользователь забыл ввести ':'
	{
		return;
	}

	while((input_num_pos = strchr(input_num_pos, ':')) || (input_num_pos = strchr(input_num_pos, ','))) // если в строке встретилось ':' или ','
	{
