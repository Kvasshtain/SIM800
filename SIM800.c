...
{
	current_state->send_uart_function(current_state->current_cmd); //с добавлением завершающих символов \r
}

// Функция отправки СМС в UART на котором сидит Sim800
inline void Sim800_WriteSMS(struct sim800_current_state *current_state)
{
	current_state->send_uart_function(current_state->send_SMS_data); //с добавлением завершающих символов \r
}
...
void sim800_AT_request(struct sim800_current_state * current_state)
{
	...
	sim800_request(current_state); // AT\r команда автонастройки скорости передачи данных SIM800 по UART
}
...
void sim800_ATplusCMGS_request(struct sim800_current_state * current_state, uint8_t * phone_number, uint8_t * SMS_data)
{
    ...
    strncat(current_state->current_cmd, "\"", 14); // добавим ковычки в конец и начало телефонного номера как того требует стандарт AT-команды
...
void sim800_ATplusCMGS_responce_handler(struct sim800_current_state * current_state)
{
	...
	else if (strstr(current_state->responce,">")) // Пришло приглашение ввести текст СМС сообщения
	{
