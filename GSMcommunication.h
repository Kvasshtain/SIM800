...
void Echo(void); // функция отправки эхо (что GSM извещатель находится в рабочем состоянии)

void Save_Alarm_Text(uint8_t type); // функция поиска в строке текста SMS тревожного сообщения выдаваемого при периодической активности на цифровом входе (наличии меандра)

void Save_Alarm_State(uint8_t state); // функция сохраниения во флеш для указанных в SMS входов нулевого активного состояния

void Save_range_Alarm_State(uint8_t state); // функция сохраниения во флеш для указанных в SMS входов нулевого активного состояния (введенных через дефис)
